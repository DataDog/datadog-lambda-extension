...
use signer::model::{S3Source, Source, Destination, S3Destination};
...
pub async fn sign(args: &SignOptions) -> Result<()> {
    ...
    upload_object(args, &key, &s3_client).await.expect("could not upload the layer");
    sign_object(&key, &signer_client).await.expect("could not sign layer");
    Ok(())
}

async fn upload_object(args: &SignOptions, key: &str, s3_client: &s3::Client) -> Result<()> {
    let body = ByteStream::from_path(Path::new(&args.layer_path)).await
...
async fn sign_object(key: &str, signer_client: &signer::Client) -> Result<String> {
    ...
    let result = signer_client.start_signing_job()
    .source(source)
