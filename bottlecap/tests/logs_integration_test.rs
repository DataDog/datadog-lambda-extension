...
async fn test_logs() {
    ...
    let invocation_deadline = bottlecap::flushing::new_invocation_deadline();
    let logs_flusher =
        LogsFlusher::new(api_key_factory, logs_aggr_handle, arc_conf.clone(), client, invocation_deadline);
