...
async fn test_metrics_flow() {
    ...
    let provider = Arc::new(Provider::new(Arc::new(Config::default()), LAMBDA_RUNTIME_SLUG.to_string(),
                                          &HashMap::from([("function_arn".to_string(), "test-arn".to_string())])));

    let metrics_aggr = Arc::new(Mutex::new(
        MetricsAggregator::<1024>::new(provider.clone())
    ...
    lambda_enhanced_metrics.increment_invocation_metric().expect("failed to increment invocation metric");
