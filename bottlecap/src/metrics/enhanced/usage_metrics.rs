...
impl UsageMetrics {
    pub fn new() -> Self {
        Self { tmp_used: 0.0, fd_use: 0.0, threads_use: 0.0 }
    }

    pub fn update_metrics(&mut self, tmp_used: Option<f64>, fd_use: Option<f64>, threads_use: Option<f64>) {
    ...
    pub fn tmp_used(&self) -> f64 {
        self.tmp_used
    }

    pub fn fd_use(&self) -> f64 {
        self.fd_use
    }

    pub fn threads_use(&self) -> f64 {
        self.threads_use
    }
}
...
impl EnhancedMetricsHandle {
    pub fn update_metrics(&self, tmp_used: Option<f64>, fd_use: Option<f64>, threads_use: Option<f64>) -> Result<(), mpsc::error::SendError<EnhancedMetricsCommand>> {
        self.tx.send(EnhancedMetricsCommand::UpdateMetrics(tmp_used, fd_use, threads_use))
    }
    ...
    pub fn pause_monitoring(&self) {
        // Send signal through watch channel for immediate notification
        let _ = self.monitoring_state_tx.send(false); // false = paused
        debug!("Enhanced metrics monitoring paused");
    }

    pub fn resume_monitoring(&self) {
        // Send signal through watch channel for immediate notification
        let _ = self.monitoring_state_tx.send(true); // true = active
        debug!("Enhanced metrics monitoring resumed");
    }
...
mod tests {
    ...
    async fn test_enhanced_metrics_service() {
        ...
        assert!(res.is_ok());
        let metrics = handle.get_metrics().await.unwrap();
        assert_eq!(metrics, UsageMetrics { tmp_used: 11.0, fd_use: 20.0, threads_use: 5.0 });
        ...
        assert!(res.is_ok());
        let metrics = handle.get_metrics().await.unwrap();
        assert_eq!(metrics, UsageMetrics { tmp_used: 50.0, fd_use: 60.0, threads_use: 70.0 });
