...
impl Trigger for SqsRecord {
    fn new(payload: Value) -> Option<Self> {
        ...
        match records {
            Some(records) => match serde_json::from_value::<SqsRecord>(records[0].clone()) {
                Ok(event) => Some(event),
                Err(e) => {
                    debug!("Failed to deserialize SQSEvent: {}", e);
                    None
                }
            },
            None => None,
    ...
    fn is_match(payload: &Value) -> bool {
        if let Some(first_record) = payload.get("Records").and_then(Value::as_array).and_then(|r| r.first()).take() {
            first_record.get("eventSource").and_then(Value::as_str).map_or(false, |s| s == "aws:sqs")
        } else {
            false
        }
    }
    ...
    fn enrich_span(&self, span: &mut Span) {
        ...
        let resource = self.event_source_arn.clone().split(':').last().unwrap_or_default().to_string();
        let start_time = (self.attributes.sent_timestamp.parse::<u64>().unwrap_or_default() as f64 * MS_TO_NS) as i64;
    ...
    fn get_arn(&self, region: &str) -> String {
        if let [_, _, _, _, account, queue_name] = self.event_source_arn.split(':').collect::<Vec<&str>>().as_slice() {
            format!(
                "arn:aws:sqs:{}:{}:{}",
