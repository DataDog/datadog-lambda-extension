...
use bottlecap::{
    ...
    lifecycle::{
        ...
        invocation::processor::{ProcessorHandle, ProcessorService},
...
async fn extension_loop_active(
    ...
    let mut event_bus: EventBus = EventBus::run();
    ...
    let (invocation_processor_service, invocation_processor) = ProcessorService::new(
    ...
    let lifecycle_listener =
        LifecycleListener::new(invocation_processor.clone(), Arc::clone(&propagator));
    ...
    handle_next_invocation(next_lambda_response, &invocation_processor);
    loop {
        ...
        match current_flush_decision {
            FlushDecision::End => {
                ...
                'flush_end: loop {
                    ...
                    biased;
                        Some(event) = event_bus.rx.recv() => {
                            if let Some(telemetry_event) = handle_event_bus_event(event, &invocation_processor, appsec_processor.as_ref(), &tags_provider, &trace_processor, &trace_agent_channel, &stats_concentrator) {
                ...
                let next_response =
                    extension::next_event(client, &aws_config.runtime_api, &r.extension_id).await;
                maybe_shutdown_event = handle_next_invocation(next_response, &invocation_processor);
            }
            FlushDecision::Continuous | FlushDecision::Periodic | FlushDecision::Dont => {
                ...
                'next_invocation: loop {
                    ...
                    biased;
                        ...
                        Some(event) = event_bus.rx.recv() => {
                            handle_event_bus_event(event, &invocation_processor, appsec_processor.as_ref(), &tags_provider, &trace_processor, &trace_agent_channel, &stats_concentrator);
                        }
                        _ = race_flush_interval.tick() => {
...
fn handle_event_bus_event(
    event: Event,
    invocation_processor: &ProcessorHandle,
    ...
    match event {
        Event::OutOfMemory(event_timestamp) => {
            invocation_processor.on_out_of_memory_error(event_timestamp);
        }
        Event::Telemetry(event) => {
            debug!("Telemetry event received: {:?}", event);
            match event.record {
                TelemetryRecord::PlatformInitStart { .. } => {
                    invocation_processor.on_platform_init_start(event.time);
                }
                TelemetryRecord::PlatformInitReport {
                    ...
                    invocation_processor.on_platform_init_report(
                        initialization_type,
                        metrics.duration_ms,
                ...
                TelemetryRecord::PlatformStart { request_id, .. } => {
                    invocation_processor.on_platform_start(request_id, event.time);
                }
                TelemetryRecord::PlatformRuntimeDone {
                    ...
                    invocation_processor.on_platform_runtime_done(
                        request_id.clone(),
                        metrics,
                        status,
                        error_type.clone(),
                        ...
                        Arc::new(SendingTraceProcessor {
                            appsec: appsec_processor.cloned(),
                ...
                TelemetryRecord::PlatformReport {
                    ...
                    invocation_processor.on_platform_report(
                        request_id.clone(),
                        metrics,
                        event.time.timestamp(),
                    );
...
fn handle_next_invocation(
    ...
    match next_response {
        Ok(NextEventResponse::Invoke {
            ...
            invocation_processor.on_invoke_event(request_id.clone());
        }
        Ok(NextEventResponse::Shutdown {
            ref shutdown_reason,
            deadline_ms,
        }) => {
            invocation_processor.on_shutdown_event();
