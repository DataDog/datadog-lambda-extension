...
impl Aggregator {
    ...
    pub fn add_batch(&self, logs: Vec<String>) {
        for log in logs {
    ...
    //         Ok(log) => self.messages.push(log),
    //         Err(e) => debug!("Failed to serialize log: {}", e),
    //     }
    // }

    pub fn get_batch(&self) -> Vec<u8> {
        debug!(
            "[jordan][aggregator] current queue size: {}",
            self.messages.len(),
        );
        ...
        for _ in 0..self.max_batch_entries_size {
            if let Some(log) = self.messages.pop() {
        ...
        buffer.extend(b"]");
        debug!("[jordan][aggregator] popping {} logs", counter,);
        buffer
    }
}
