...
impl TraceAgent {
    ...
    pub async fn start(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let invocation_processor = Arc::clone(&self.invocation_processor);
        ...
        let make_svc = make_service_fn(move |_| {
            let invocation_processor = Arc::clone(&invocation_processor);
            ...
            for span in t {
                if span.resource == INVOCATION_SPAN_RESOURCE {
                    if let Some(request_id) = span.meta.get("request_id") {
                        let mut guard = invocation_processor.lock().await;
