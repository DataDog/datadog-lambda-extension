...
//! shared by both `[[bin]]` targets: the Lambda extension (`bottlecap`) and
//! `bottlecap-test-mode`, which builds the same pipeline but attaches its
//! own router extension before spawning the agent.
//!
