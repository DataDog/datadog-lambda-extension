...
pub fn resolve_secrets(config: Config) -> Result<Config> {
    if !config.api_key.is_empty() {
        ...
        return if !config.api_key_secret_arn.is_empty() {
            let before_manual = Instant::now();

            let resolved_key = manual_decrypt(config.api_key_secret_arn.clone(), AwsConfig {
            ...
            Err(Error::new(std::io::ErrorKind::InvalidInput, "No API key or secret ARN found".to_string()))
        };
    }
}
...
fn build_get_secret_signed_headers(aws_config: &AwsConfig, json_body: &Value, t: DateTime<Utc>) -> HeaderMap {
    ...
    let credential_scope = format!("{}/{}/{}/aws4_request", date_stamp, aws_config.region, service);
    ...
    let signing_key = get_aws4_signature_key(&aws_config.aws_secret_access_key, &date_stamp, aws_config.region.as_str(), service);
    ...
    headers.insert("x-amz-security-token", HeaderValue::from_str(&aws_config.aws_session_token).unwrap());
    headers
}
...
fn get_aws4_signature_key(key: &str, date_stamp: &str, region_name: &str, service_name: &str) -> Vec<u8> {
...
mod tests {
    ...
    fn test_build_get_secret_signed_headers() {
        let time = Utc.from_utc_datetime(&NaiveDateTime::parse_from_str("2024-05-30 09:10:11", "%Y-%m-%d %H:%M:%S").unwrap());
        let headers = build_get_secret_signed_headers(&AwsConfig {
        ...
        }, &serde_json::json!({ "SecretId": "arn:aws:secretsmanager:region:account-id:secret:secret-name"}), time);
        ...
        for (k, v) in expected_headers.iter() {
