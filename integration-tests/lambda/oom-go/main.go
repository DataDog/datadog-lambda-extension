...
// to force physical commit. On a 192 MB Lambda this immediately exceeds the
// cgroup memory limit and the kernel SIGKILLs the process, producing a
// PlatformReport with max_memory_used_mb == memory_size_mb. The Go runtime
...
func handler() error {
	...
	log.Println("did not OOM — unexpected") // unreachable on a 192 MB Lambda
	return nil
}
