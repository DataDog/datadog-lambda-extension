...
describe('Base .NET Lambda Integration Test', () => {
  const DOTNET_FUNCTION_NAME = `integ-${getIdentifier()}-base-dotnet-dotnet-function`;

  it('should invoke .NET Lambda successfully and receive logs and traces in Datadog', async () => {

    // Step 1: Invoke the Lambda function with a cold start
    console.log(`Invoking Lambda function: ${DOTNET_FUNCTION_NAME}`);
    const result = await invokeLambdaAndGetDatadogData(DOTNET_FUNCTION_NAME, {}, true);
    ...
    console.log('✅ .NET Lambda test passed! Logs successfully sent via extension and appeared in Datadog');
